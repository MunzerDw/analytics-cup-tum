...
balanced_train <- shuffled[1:20000,]
balanced_val <- shuffled[20001:23000,]
...
model <- randomForest(OFFER_STATUS ~ . - CUSTOMER_ID,
                              data = balanced_train, ntree = 1000, mtry = 4)
...
submission_file_df <- submission_file_df %>% 
    rename(
        id = TEST_SET_ID,
        prediction = OFFER_STATUS
    )
